<template>
<div>
    <div class="selectgroup" role="radio" :aria-checked="modelValue === ''" @click="$emit('update:modelValue', '')">
    ...
    <template v-for="node in tree" :key="node.type === 'folder' ? 'folder:'+node.folder.id : 'feed:'+node.feed.id">
        <template v-if="node.type === 'folder'">
            <div class="selectgroup mt-1" role="radio" :aria-checked="modelValue === 'folder:'+node.folder.id" @click="$emit('update:modelValue', 'folder:'+node.folder.id)">
                <div class="selectgroup-label d-flex align-items-center w-100">
                    <div @click.stop="$emit('toggle-folder', node.folder)" class="m-n1 p-1">
                        <v-icon class="mr-2" :class="{expanded: node.folder.is_expanded}" name="chevron-right" />
                    </div>
            ...
            <div v-show="node.folder.is_expanded" class="mt-1 pl-3">
                <div class="selectgroup" role="radio" :aria-checked="modelValue === 'feed:'+feedNode.feed.id" @click="$emit('update:modelValue', 'feed:'+feedNode.feed.id)"
                        v-for="feedNode in node.feeds">
                    <div class="selectgroup-label d-flex align-items-center w-100">
                        <v-icon class="mr-2" name="rss" v-if="!feedNode.feed.icon" />
                        <span class="icon mr-2" v-else><img :src="feedNode.feed.icon" alt="" loading="lazy"></span>
                        <span class="flex-fill text-left text-truncate">{{ feedNode.feed.title }}</span>
                        <span class="counter text-right">{{ filteredFeedStats[feedNode.feed.id] || '' }}</span>
        ...
        <template v-else-if="node.type === 'feed'">
            <div class="selectgroup" role="radio" :aria-checked="modelValue === 'feed:'+node.feed.id" @click="$emit('update:modelValue', 'feed:'+node.feed.id)">
                <div class="selectgroup-label d-flex align-items-center w-100">
                    <v-icon class="mr-2" name="rss" v-if="!node.feed.icon" />
                    <span class="icon mr-2" v-else><img :src="node.feed.icon" alt="" loading="lazy"></span>
